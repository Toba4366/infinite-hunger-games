...
# criterion at the last lesson then keeps training, with the same weights or population, for up to 16,000 more
# iterations or 14 hours, so the record shows how long every method needs to graduate the whole curriculum.
...
python3 -u experiments/run_comparison.py --methods imitation,genetic,neat,reinforce,ppo --pairs --curriculum lessons \
  --iterations 150 --until-win 0.5 --window 5 --extend-iterations 16000 --extend-hours 14 \
...
python3 -u experiments/run_comparison.py --methods imitation,reinforce --warm --curriculum lessons --sizes 16,64x32,128x64 \
  --iterations 150 --until-win 0.5 --window 5 --extend-iterations 16000 --extend-hours 8 \
