...
**The curriculum.** `observe(mean_score, win_rate=0.0)` returns `True` on promotion. With the default `promote_on="win_rate"`, promotion happens when the last `window` win rates average at least `win_threshold`, or when `max_iterations_per_stage` iterations have passed in the stage and that limit is above 0. With `promote_on="score"` the mean scores are judged against `threshold` instead. See [../training/common.md](../training/common.md).
...
**`assert genome.forward(np.zeros(VECTOR_SIZE)).shape == (MENU_SIZE,)`.** A minimal genome evaluates to one score per menu item. This is the first forward pass, so it also compiles the evaluation plan. A failure means the forward pass does not produce the output layer in order.
...
**`rebuilt = NeatGenome.from_dict(genome.to_dict())` then `assert np.allclose(rebuilt.forward(ones), genome.forward(ones))`.** The JSON round trip keeps every node, connection, weight and enabled flag, and the rebuilt genome's freshly compiled plan gives the same numbers as the mutated genome's. Because `mutate` invalidates the plan after every weight change, the original's plan is fresh too. A failure means champion files or `LearnerSpec` dictionaries would rebuild a different network, or that a stale plan survived a mutation.
...
### `test_curriculum_promotes_on_wins_or_timeout()`

**Setup.** `Curriculum(CurriculumConfig(opponents=(1, 3, 7), win_threshold=0.5, window=2, max_iterations_per_stage=4))`. The default `promote_on="win_rate"` applies.

**`assert curriculum.opponents == 1`.** Stage 0 is the first entry.

**`assert not curriculum.observe(0.0, win_rate=0.0)`.** One win rate is fewer than `window`, so no promotion.

**`assert curriculum.observe(0.0, win_rate=1.0) is True and curriculum.opponents == 3`.** The window is now `[0.0, 1.0]`, mean 0.5, which reaches `win_threshold`. Promotion to stage 1. The mean score is 0.0 both times and plays no part. A failure means the mean test, the window length test, or the choice of metric is wrong.

**`assert not any(curriculum.observe(9.0, win_rate=0.0) for _ in range(3))`.** Three iterations in stage 1 with a high score but no wins: the win rate never reaches 0.5 and three is fewer than `max_iterations_per_stage`. A failure here usually means the score is being judged instead of the win rate.

**`assert curriculum.observe(9.0, win_rate=0.0) is True and curriculum.opponents == 7 and curriculum.finished`.** The fourth iteration times the stage out. Stage 2 is the last, so `finished` is true. A failure means the timeout is off by one or `finished` does not recognise the last stage.

**`never = Curriculum(CurriculumConfig(opponents=(1, 3), win_threshold=0.5, window=1, max_iterations_per_stage=0))` then `assert not any(never.observe(0.0, win_rate=0.0) for _ in range(50))`.** With the limit at 0 there is no timeout: fifty losing iterations never promote. A failure means `0` is being treated as "promote at once" or as some other limit.

**`by_score = Curriculum(CurriculumConfig(opponents=(1, 3), promote_on="score", threshold=1.0, window=1))` then `assert by_score.observe(2.0, win_rate=0.0) is True`.** With `promote_on="score"` a single mean score of 2.0 clears `threshold=1.0` in a window of one, and the zero win rate is ignored. A failure means `promote_on` is not switching the judged metric.
...
**`assert np.isfinite(stats.mean_score) and stats.mean_length >= 0 and 0.0 <= stats.win_rate <= 1.0`.** The three headline numbers are sane. A `NaN` mean score would poison every overlay chart; a negative length or a win rate outside 0 to 1 means the wrong quantity was stored. The win rate is game-level now, so a value above one sixth is allowed and expected when the learner wins.
...
**Setup, part one.** `Curriculum(CurriculumConfig(opponents=(2, 5), win_threshold=99.0, window=1, max_iterations_per_stage=1))`: an unreachable win threshold and a one-iteration timeout, so every iteration promotes by timeout. `GeneticTrainer(small(), TrainingConfig(brain_name="neural", population_size=4, generations=2, rounds_per_generation=1, validation_games=0, learners_per_game=2, seed=0), curriculum=curriculum)`, then `trainer.run()`.
...
- To see the promotion rule fail, change `win_rate=1.0` in the second test to `0.9`: the window mean becomes 0.45 and the second assertion fails.
...
- **The genetic curriculum test only promotes by timeout.** `win_threshold=99.0` can never be reached, and `GeneticTrainer` passes no win rate to `observe` anyway, so the two promotions come from `max_iterations_per_stage=1`. The test proves the roster follows the stage, not that the GA can earn a stage.
...
- **`validation_games=0`** in the fourth test means `val_score` and `val_win_rate` are 0.0 and no showcase is recorded; that is fine because only `opponents` and `num_players` are checked.
