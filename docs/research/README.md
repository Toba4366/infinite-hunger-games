...
**Promotion rule.** After every iteration the trainer calls `curriculum.observe(mean_score, win_rate, survival)`, passing the validation win rate when it plays validation games and the training win rate otherwise, and the share of the game its copies survived; the lesson decides which of the three it is judged on. It returns `True`, and the trainer logs a `curriculum` event, when the learner has won at least half of those games over the last five iterations. There is no timeout by default: a learner that cannot beat one opponent stays against one opponent, which is a finding, not a bug. The trainer then rebuilds its config with `num_players = learners + opponents`, so the first stage is a 7-tribute game (6 learners and 1 opponent) and the last is the full 29 (6 and 23). The stage and opponent count are on every `IterationStats`, and `plots/curriculum.png` in a run folder draws the ladder. Genetic, NEAT, REINFORCE and PPO all judge promotion on validation wins. Imitation ignores the curriculum because it never plays training games.

**How the curriculum got here.** It has been rewritten three times, each time because a run showed a hole. (1) The first version promoted on **mean score**: the learner moved up as soon as its average return over five iterations passed 3.0. Networks that had learned to survive but never to win were promoted into fields they could not beat, and the score fell at every rung. (2) Promotion moved to **wins**: the learner must win a majority of its validation games over five iterations, with no timeout. (3) The comparison's win criterion was found to count the five wins that earned the final promotion as wins at the final stage, so a warm REINFORCE variant was declared "reached" the instant it reached 23 opponents; the criterion now requires every iteration of its window to have been played at the final stage. (4) The first full experiment ([../results/full_methods/paper.md](../results/full_methods/paper.md)) showed the remaining hole: every cold start died of thirst long before it met its single opponent, because no rung teaches drinking. That produced the **lesson curriculum** below.

**The lesson curriculum.** `CurriculumConfig.lessons()` replaces the opponent ladder with eight lessons, each a `Stage` with its own opponents, rules and promotion metric:
...
The first two lessons teach what imitation pretraining otherwise supplies (drink, eat, move with the circle), which sets up a clean experiment: a cold start on the lessons against a warm start on the ladder. The last lesson varies the arena from game to game, so a champion that only knows one layout is found out before the tournament. Every trainer rebuilds its config per stage with `stage_config` (roster size plus the lesson's overrides) and per training episode with `episode_config` (one of the lesson's variants chosen by the episode seed); validation games use the stage's base rules. Select it with `run_comparison.py --curriculum lessons` or `Variant(..., curriculum="lessons")`.
...
**What `results.csv` contains.** One row per variant: `variant`, `method`, `iterations`, `train_seconds`, `final_mean_score`, `best_val_score`, `reached_criterion`, `iterations_to_criterion`, `seconds_to_criterion`, `extended_iterations`, `final_val_win_rate`, `tournament_score`, `tournament_win_rate`, `tournament_survival`, `tournament_kills`, `lines_of_code`. `summary.json` adds the tournament dictionary and every learning curve; `results_table.tex` is the same table for a paper.
