...
**Files here:** [paper.md](paper.md) (the write-up as a research paper), [gifs/index.md](gifs/index.md) (one tournament game per champion as a GIF), [analysis/stats.md](analysis/stats.md) (intervals, tests and trends), [report.md](report.md) (generated by the run), [results.csv](results.csv) (every number), [summary.json.gz](summary.json.gz) (the full learning curves, gzipped because 1,150 iterations of eight variants is 3.4 MB), [config.json](config.json) (the exact configuration), and the charts named below.
