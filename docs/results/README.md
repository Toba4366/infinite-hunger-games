...
Each page states the exact command, the machine, the tables, the claims the run supports, and its limitations. To reproduce a run, use the command on its page; the run folder it writes has the same file names as the page's copies. See [../research/README.md](../research/README.md) for how the experiments are designed and [../output.md](../output.md) for what every file in a run folder means.
